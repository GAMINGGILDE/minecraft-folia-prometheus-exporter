...
Für fortlaufend akkumulierte Event-Counter gilt die eng begrenzte Ergänzung aus
[ADR 0014](0014-event-counters.md): Eventhandler erhöhen
...
Prozessinstrumentierung gilt die eng begrenzte Ausnahme aus
[ADR 0012](0012-direct-jvm-process-instrumentation.md). Minecraft-Daten bleiben
ohne Ausnahme snapshotbasiert.

`ImmutableSnapshot<T>` konkretisiert dies mit einem eindeutigen `Instant` und
einer defensiv kopierten, unveränderlichen Werteliste. Der Werttyp
